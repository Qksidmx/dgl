...
//            }
        ...
        void ParseFileOpts(const std::string &filename, const std::string &secondary_filename) {
            ...
            if (f == nullptr) {
                ...
                if (f == nullptr) {
                    GFS_LOG_WARNING("{} not exists, using inline options",filename);
		    ParseInlineOpts();
		    return;
                }
            }
        ...
        void PrintOpts(bool logging) {
            ...
            for (auto &iter : conf) {
                if (logging) {
                    GFS_LOG_INFO("[{}] => [{}]", iter.first, iter.second);
                } else {
