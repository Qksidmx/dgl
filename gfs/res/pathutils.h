#ifndef GFS_GRAPHCHI_PATH_UTILS
#define GFS_GRAPHCHI_PATH_UTILS
#include <stdio.h>
#include <libgen.h>
#include <string.h>
#include <unistd.h>
...
namespace gfs{
    ...
    public:
        ...
        static
        gfs::Status CreateDirIfMissing(const std::string &dir) {
            gfs::Status s;
            char tmp[PATH_MAX];
        ...
        static
        gfs::Status CreateFile(const std::string &fname) {
            int f = open(fname.c_str(), O_WRONLY | O_CREAT, S_IROTH | S_IWOTH | S_IWUSR | S_IRUSR);
            if (f < 0) {
                return gfs::Status::IOError(fmt::format("Can NOT create file: {}, error: {}",
                                                        fname, strerror(errno)));
            }
            close(f);
            return gfs::Status::OK();
        }
    
        static
        gfs::Status CreateFileIfMissing(const std::string &fname) {
            if (!FileExists(fname)) {
        ...
        static
        inline
        gfs::Status TruncateFile(const std::string &fname, off_t length) {
            const int iRet = truncate(fname.c_str(), length);
            if (iRet == 0) {
                return gfs::Status::OK();
            } else {
                return gfs::Status::IOError(fmt::format("Can NOT truncate file: {}, error: {}",
                                                        fname, strerror(errno)));
            }
        }
        ...
        static
        inline
        gfs::Status RemoveFile(const std::string &fname) {
            struct stat buf;
            if (lstat(fname.c_str(), &buf) < 0) {
                return gfs::Status::IOError(fmt::format("Fail to get stat of {}", fname));
            }
            // 如果是目录, 先删除目录下的所有文件
            ...
            if (iRet == 0) {
                ...
                return gfs::Status::IOError(fmt::format("Can NOT remove file: {}", fname));
            }
        }
    
        static
        inline
        gfs::Status RenameFile(const std::string &oldfile, const std::string &newfile) {
            const int iRet = rename(oldfile.c_str(), newfile.c_str());
            if (iRet < 0) {
                return gfs::Status::IOError(fmt::format("Fail rename file: {} -> {}, error: {}",
                                                        oldfile, newfile,
                                                        strerror(errno)));
            } else {
                return gfs::Status::OK();
