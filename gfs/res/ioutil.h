...
template <typename T>
gfs::Status preada(int f, T * tbuf, size_t nbytes, size_t off) {
    size_t nread = 0;
    char * buf = (char*)tbuf;
    while(nread<nbytes) {
#ifdef SKG_LIMIT_DEF_MAX_READ_BYTES
...
template <typename T>
gfs::Status pwritea(int f, T * tbuf, size_t nbytes, size_t off) {
    size_t nwritten = 0;
    assert(f>0);
    char * buf = (char*)tbuf;
...
void checkarray_filesize(std::string fname, size_t nelements) {
    ...
    if (f < 1) {
        GFS_LOG_ERROR("Error initializing the data-file: {} error:", fname, strerror(errno));
    }
    assert(f>0);
    int err = ftruncate(f, nelements * sizeof(T));
    if (err != 0) {
        GFS_LOG_ERROR("Error in adjusting file size: {} to size: {} error: {}",
...
template <typename T>
gfs::Status writea(int f, T * tbuf, size_t nbytes) {
    size_t nwritten = 0;
    char * buf = (char*)tbuf;
    while(nwritten<nbytes) {
