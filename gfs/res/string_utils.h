#ifndef GFS_STRINGUTILS_HPP
#define GFS_STRINGUTILS_HPP
...
public:
    ...
    static
    std::vector<Slice>
            split(const Slice &str, const char pattern,
                  const int max_split=-1);
    ...
    inline static
    bool IsValidName(const Slice &slice) {
        if (slice.empty()) {
            return false;
        }
    ...
    inline static
    uint32_t ParseUint32(const std::string &value, gfs::Status *s) {
        uint64_t num = 0;
        try {
            num = ParseUint64(value);
        } catch (std::exception const &e) {
            *s = gfs::Status::InvalidArgument(fmt::format("invalid long: {}", value));
            return 0;
        }
        if ((num >> 32LL) == 0) {
            ...
            if (s != nullptr) {
                *s = gfs::Status::InvalidArgument(fmt::format("out of range: {}", value));
            }
