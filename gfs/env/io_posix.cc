...
#if defined(SKG_FALLOCATE_PRESENT) && !defined(TRAVIS)
...
CWritableFile::CWritableFile(const std::string &fname, FILE *fd, const EnvOptions &options)
        : filename_(fname), fd_(fd), filesize_(0) {
    // 设置缓冲区大小
    setvbuf(fd_, nullptr, _IOFBF, options.writable_file_max_buffer_size);
}

CWritableFile::~CWritableFile() {
    if (fd_ != nullptr) {
        Close();
    }
}

Status CWritableFile::Append(const Slice &data) {
    const char *src = data.data();
    size_t left = data.size();
    while (left != 0) {
        ssize_t done = fwrite(src, 1, left, fd_);
        if (done < 0) {
            if (errno == EINTR) {
                continue;
            }
...
Status CWritableFile::Truncate(uint64_t size) {
    Status s;
    int r = ftruncate(fileno(fd_), size);
    if (r < 0) {
        s = IOError("While ftruncate file to size " + ToString(size), filename_,
                    errno);
    } else {
...
Status CWritableFile::Close() {
    if (fd_ != nullptr) {
        if (fclose(fd_) < 0) {
            return IOError("While closing file after writing", filename_, errno);
        }
        fd_ = nullptr;
    }
    return Status::OK();
...
// write out the cached data to the OS cache
Status CWritableFile::Flush() {
    return Status();
}

Status CWritableFile::Sync() {
    if (fdatasync(fileno(fd_)) < 0) {
        return IOError("While fdatasync", filename_, errno);
    }
    return Status::OK();
}

Status CWritableFile::Fsync() {
...
uint64_t CWritableFile::GetFileSize() {
    return filesize_;
}

#ifdef OS_LINUX
size_t CWritableFile::GetUniqueId(char *id, size_t max_size) const {
    return PosixHelper::GetUniqueIdFromFile(fileno(fd_), id, max_size);
}
#endif
}
