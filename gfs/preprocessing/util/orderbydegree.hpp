...
#define DEF_GRAPHCHI_ORDERBYDEGREE
...
namespace skg {
    ...
    int order_by_degree(std::string &base_filename, int nshards, metrics &m) {
        ...
        close(f);
        SKG_LOG_INFO("Starting to order by degrees, number of vertices: {}", nverts);
        ...
        SKG_LOG_INFO("Created translation table. ", "");
        ...
        if (df < 0) {
            SKG_LOG_ERROR("Could not write vertex map: {} error: {}", translate_table_file, strerror(errno));
        }
        assert(df >= 0);
        pwritea(df, translate_table, nverts * sizeof(vid_t), 0);
        close(df);
        
        SKG_LOG_INFO("Translation map saved into file: {}", translate_table_file);
        ...
        for(int p=0; p<nshards; p++) {
            SKG_LOG_INFO("Reprocessing shard {}", p);
            ...
            memory_shard<int, EdgeDataType, special_sharding_vertex<int, EdgeDataType> > memshard(
                    iomgr, filename_shard_edata<EdgeDataType>(base_filename, p, nshards),
