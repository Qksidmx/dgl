...
#define logbuf(lvl,buf,len)                 \
    (log_dispatch<((lvl) >= OUTPUTLEVEL)>::exec((lvl), __FILE__, __func__ ,__LINE__,(buf),(len)))
#define logstream(lvl)                      \
    (log_stream_dispatch<((lvl) >= OUTPUTLEVEL)>::exec((lvl), __FILE__, __func__ ,__LINE__) )
#endif

static const char* messages_lvl[] = {
    "D:",
    "I:",
    "W:",
    "E:",
  ...
  */
  ...
  file_logger& operator<<(const char* a) {
    ...
    logger_impl::streambuff_tls_entry* streambufentry =
            reinterpret_cast<logger_impl::streambuff_tls_entry*>(pthread_getspecific(streambuffkey));
    if (streambufentry != NULL) {
      std::stringstream& streambuffer = streambufentry->streambuffer;
      bool& streamactive = streambufentry->streamactive;
  ...
  void set_log_level(int new_log_level) {
    ...
    bool set_log_file(const std::string &file) {
...
#define WHITE   7
    ...
    void textcolor(FILE* handle, LogColorAttr attr, LogColor fg)
    {
        char command[13];
        /* Command is the control command to the terminal */
        sprintf(command, "%c[%d;%dm", 0x1B, static_cast<int>(attr), static_cast<int>(fg) + 30);
    ...
    void _log(int lineloglevel,const char* file,const char* function,
        ...
        if (lineloglevel >= 0 && lineloglevel <= 3 && lineloglevel >= log_level){
            ...
            const std::string timeStr = fmt::format("{:%m/%d %H:%M:%S}", *std::localtime(&t));
            int byteswritten = snprintf(str,1024, "%s [%s] %s(%s:%d): ",
                                        messages_lvl[lineloglevel], timeStr.c_str(), file,
                                        function,line);
            ...
            if (log_to_console) {
                ...
                if (lineloglevel == LOG_FATAL) {
                    textcolor(stderr, LogColorAttr::BRIGHT, LogColor::RED);
                }
                else if (lineloglevel == LOG_ERROR) {
                    textcolor(stderr, LogColorAttr::BRIGHT, LogColor::RED);
                }
                else if (lineloglevel == LOG_WARNING) {
                    textcolor(stderr, LogColorAttr::BRIGHT, LogColor::GREEN);
                }
#endif
                std::cerr << str;;
#ifdef COLOROUTPUT
    ...
    void _logbuf(int lineloglevel,const char* file,const char* function,
        ...
        if (lineloglevel >= 0 && lineloglevel <= 3 && lineloglevel >= log_level){
            ...
            int headerlen = snprintf(NULL, 0, "%s%s(%s:%d): ",
                                     messages_lvl[lineloglevel], file, function, line);
    ...
    void _lograw(int lineloglevel, const char* buf, int len) {
        ...
        if (log_to_console) {
            ...
            if (lineloglevel == LOG_FATAL) {
                ...
                textcolor(stderr, LogColorAttr::BRIGHT, LogColor::YELLOW);
            }
...
//                << "(" << function << ":" <<line<<"): ";
                    std::time_t t = std::time(nullptr);
                    const std::string timeStr = fmt::format("{:%m/%d %H:%M:%S}", *std::localtime(&t));
                    streambuffer << fmt::format("{} [{}] {}({}:{}): ",
                                                messages_lvl[lineloglevel], timeStr, file,
                                                function, line);
                }
                streamactive = true;
