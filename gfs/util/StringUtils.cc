...
std::vector<skg::Slice>
        StringUtils::split(const skg::Slice &str, const char pattern, const int max_split) {
    std::vector<skg::Slice> result;
    assert(max_split == -1 || max_split >= 0);  // max_split 不应为-1以外的负值
    ...
    while (true) {
        ...
        if (max_split != -1 && numPattern == static_cast<size_t>(max_split)) {
            // printf("[Max] Push `%s`\n", str.substr(begPos).c_str());
#if __cplusplus < 201103L
            result.push_back(skg::Slice(str.data_ + begPos));
#else
        ...
        //         str.substr(begPos, endPos - begPos).c_str());
#if __cplusplus < 201103L
        result.push_back(skg::Slice(str.data_ + begPos, endPos - begPos));
#else
