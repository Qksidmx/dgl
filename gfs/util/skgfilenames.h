...
namespace skg {
    ...
    public:
        ...
        static std::string VARIABLE_IS_NOT_USED sub_partition(
                const std::string &dirname, 
                uint32_t shard_id, uint32_t partition_id,
                const skg::interval_t &interval, const skg::EdgeTag_t tag) {
        ...
        static std::string VARIABLE_IS_NOT_USED shardtree(
                const std::string &dirname, 
                uint32_t shard_id,
                const skg::interval_t interval=interval_t(0,0)) {
    ...
    public:
        ...
        static std::string VARIABLE_IS_NOT_USED degree(const std::string &basefilename) {
            return FILENAME::vertex_attr_data(basefilename, SKG_GLOBAL_LABEL, SKG_VERTEX_COLUMN_NAME_DEGREE);
        }
        ...
        static std::string VARIABLE_IS_NOT_USED vtag(const std::string &basefilename) {
            return FILENAME::vertex_attr_data(basefilename, SKG_GLOBAL_LABEL, SKG_VERTEX_COLUMN_NAME_TAG);
        }
        ...
        static std::string VARIABLE_IS_NOT_USED edge_status(const std::string &basefilename,
                uint32_t shard_id, uint32_t partition_id,
                const skg::interval_t &interval, const skg::EdgeTag_t tag, const std::string& statusname) {
...
#ifdef IO_TRACK
            SKG_LOG_DEBUG("read open {}", filename_cstr);
#endif
            int fd = open(filename_cstr, O_RDONLY);
            if (fd < 0) {
                SKG_LOG_DEBUG("err open [{}]", filename_cstr);
                exit(-1);
            } 
#ifdef IO_TRACK
            SKG_LOG_DEBUG("read open {} with fd {}", filename_cstr, fd);
#endif
            return fd;
        }
...
#ifdef IO_TRACK
            SKG_LOG_DEBUG("write open {}", filename_cstr);
#endif
...
#ifdef IO_TRACK
            SKG_LOG_DEBUG("write open {} with fd {}", filename_cstr, fd);
#endif
            return fd;
        }

        static bool status_ftruncate(const char* filename_cstr, size_t file_size) {
            SKG_LOG_DEBUG("truncate: {} size={}", filename_cstr, file_size);
