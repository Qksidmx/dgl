...
public:
    ...
    Status Open() override {
        ...
        if (m_flags & MMAP_USE_LOCKED) { mmap_flags |= MAP_LOCKED; }
        ...
        if (m_mapped_edges == MAP_FAILED) {
            SKG_LOG_WARNING("Can NOT mmap {}, error: {}({}), falling back to default-flags",
                            m_filename, strerror(errno), errno);
            // 回退到默认的标志位再尝试
            m_mapped_edges = static_cast<PersistentEdge *>(mmap(
    ...
    void Close() override {
        ...
        if (m_mapped_edges != nullptr && m_mapped_edges != MAP_FAILED) {
            ...
            if (iRet != 0) {
                SKG_LOG_ERROR("error munmap, {}", strerror(errno));
            }
            m_mapped_size = 0;
